...
void main()
{
	...
	vec3 color = GetDiffuse(tes_in.fragPos,tes_in.height,N,tes_in.texCoord,slope);
	vec3 normalTex = GetNormal(tes_in.fragPos,tes_in.height,N,tes_in.texCoord,slope);
	vec3 color = GetDiffuse(tes_in.fragPos,tes_in.height,N,tes_in.texCoord,slope);
	
	vec3 viewDir = normalize(cameraPos.xyz - tes_in.fragPos);
	float specv = 0.1f;
	...
	vec3 normal = getNormalFromMap(normalTex, tes_in.texCoord, tes_in.fragPos,N);
	...
	vec3 lightSpec = float3(0.3,0.3,0.3);
